...
if(empty($_POST['email']) || empty($_POST['name']) || empty($_POST['number']) || empty($_POST['amount'])){
  echo "Please provide all credentials.";
  exit();
  
}
$merchantId = 'PGTESTPAYUAT'; // sandbox or test merchantId
$apiKey="099eb0cd-02cf-4e2a-8aca-3e6c6aff0399"; // sandbox or test APIKEY
$redirectUrl = 'http://localhost:8000/paymentCheck.php';
//$redirectUrl = '/pg/v1/status/.';
// Set transaction details
$order_id = uniqid(); 
$name= htmlspecialchars($_POST['name']);
$email=htmlspecialchars($_POST['email']);
$mobile=htmlspecialchars($_POST['number']);
$amount = htmlspecialchars($_POST['amount']); // amount in INR
